...
test('OLSY-002 - numbet og items in the cart', async ({ page }) => {
  ...
  await expect(page.locator(`[class='pay-container'] [class='cart-preview']`)).toBeVisible();


  // how to write the correct locators for the below expects?
  await expect(page.locator(`[class='list-item'] span["Cappuccino"]`)).toContainText('x 1'); 
  
  await expect(page.locator(`[class='list-item'] span["Espresso"]`)).toContainText('x 1');

  await page.getByLabel('Add one Mocha').click();
  await page.getByLabel('Add one Mocha').click();
  await expect(page.locator(`[class='list-item'] span["Mocha"]`)).toContainText('x 3');
})
